...
업무프로그램 하위는 계약관리, 관리비내역, 영수증관리, 업무보고/현황, 기초정보입니다. 감독만 **DB 업데이트**로 원본 `login.do` 세션을 읽어 Mongo(없으면 로컬 JSON)에 복사합니다. 원본에는 조회만 하고 쓰기·삭제는 하지 않습니다. CEO만 **현황관리**에서 복사본 합계(계약자 수, 납부자 수, 금액, 미납 건수·금액)를 봅니다. 원본 사용자 비밀번호 목록은 가져오지 않습니다.
