...
업무프로그램 하위는 계약관리, 관리비내역, 영수증관리, 업무보고/현황, 기초정보입니다. 그 다음 1차는 **현황관리**(슈퍼바이저·CEO), 그다음 **DB 업데이트**(슈퍼바이저)입니다. **계약·관리비·영수증 목록과 현황관리**는 같은 복사본(Mongo, 비어 있으면 `data/work-*.local.json`)을 읽습니다. 숫자를 지어내지 않습니다. 감독의 **DB 업데이트** 버튼과 매일 새벽 1시(Asia/Seoul) 타이머, Render Cron이 치는 `/api/cron/work-sync`는 같은 읽기 전용 복사입니다. 원본 로그인은 `CEMETERY_SOURCE_ID` / `CEMETERY_SOURCE_PASSWORD`만 씁니다. 화면에 아이디·비밀번호를 받지 않습니다. 값이 없으면 복사를 건너뛰고 비밀은 로그에 남기지 않습니다. 원본에는 조회만 하고 쓰기·삭제는 하지 않습니다. 원본 사용자 비밀번호 목록은 가져오지 않습니다.
