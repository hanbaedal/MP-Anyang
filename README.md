...
업무프로그램 하위는 계약관리, 관리비내역, 영수증관리, 업무보고/현황, 기초정보입니다. **계약·관리비·영수증 목록과 현황관리**는 같은 복사본(Mongo, 비어 있으면 `data/work-*.local.json`)을 읽습니다. 숫자를 지어내지 않습니다. 감독만 **DB 업데이트**로 원본 `login.do` 세션을 읽어 그 복사본을 갱신합니다. 원본 로그인은 Render의 `CEMETERY_SOURCE_ID` / `CEMETERY_SOURCE_PASSWORD`만 씁니다. 화면에 아이디·비밀번호를 받지 않습니다. 원본에는 조회만 하고 쓰기·삭제는 하지 않습니다. 슈퍼바이저와 CEO만 **현황관리**에서 복사본 합계를 봅니다. 원본 사용자 비밀번호 목록은 가져오지 않습니다.
