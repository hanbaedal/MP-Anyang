...
export function InquiryForm() {
  ...
  return (
    <form onSubmit={onSubmit} className="max-w-lg space-y-4 rounded-xl border bg-card p-5 shadow-sm">
      <div className="space-y-2">
        <label htmlFor="inquiry-name" className="text-sm font-medium">
          이름
        </label>
        <input id="inquiry-name" name="name" required minLength={2} maxLength={40} autoComplete="name" className={field} />
      </div>
      <div className="space-y-2">
      ...
      <div className="space-y-2">
        ...
        <textarea id="inquiry-message" name="message" required minLength={5} maxLength={2000} rows={6} className={`${field} h-auto py-2`} />
      </div>
